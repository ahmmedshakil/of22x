...
namespace Foam
{
defineTypeNameAndDebug(multiCellCuts, 0);
}
...
Foam::label Foam::multiCellCuts::findPartIndex
(
...
Foam::boolList Foam::multiCellCuts::expand
(
    const label size,
...
Foam::scalarField Foam::multiCellCuts::expand
(
    const label size,
    const labelList& labels,
...
Foam::label Foam::multiCellCuts::firstUnique
(
    const labelList& lst,
    const Map<label>& map
)
...
Foam::label Foam::multiCellCuts::edgeEdgeToFace
(
    const label cellI,
    const label edgeA,
    const label edgeB
) const
{
    ...
    WarningIn
    (
        "Foam::multiCellCuts::edgeEdgeToFace"
        "(const label cellI, const label edgeA,"
        "const label edgeB) const"
    )   << "multiCellCuts : Cannot find face on cell "
...
Foam::label Foam::multiCellCuts::edgeVertexToFace
(
    const label cellI,
    const label edgeI,
    const label vertI
) const
{
    ...
    WarningIn
    (
        "Foam::multiCellCuts::edgeVertexToFace"
...
Foam::label Foam::multiCellCuts::vertexVertexToFace
(
    const label cellI,
    const label vertA,
    const label vertB
) const
{
    ...
    WarningIn("Foam::multiCellCuts::vertexVertexToFace")
        << "multiCellCuts : Cannot find face on cell "
...
void Foam::multiCellCuts::calcFaceCuts() const
{
    if (faceCutsPtr_)
    {
        FatalErrorIn("multiCellCuts::calcFaceCuts()")
    ...
    for (label faceI = 0; faceI < mesh().nFaces(); faceI++)
    {
        ...
        if (allVerticesCut)
        {
            WarningIn("Foam::multiCellCuts::calcFaceCuts() const")
...
Foam::label Foam::multiCellCuts::findEdge
(
    const label faceI,
    const label v0,
    const label v1
...
Foam::label Foam::multiCellCuts::loopFace
(
    const label cellI,
...
bool Foam::multiCellCuts::walkPoint
(
    const label cellI,
...
bool Foam::multiCellCuts::crossEdge
(
    const label cellI,
    const label startCut,
    const label faceI,
...
bool Foam::multiCellCuts::addCut
(
    const label cellI,
    const label cut,
    label& nVisited,
...
bool Foam::multiCellCuts::walkFace
(
    const label cellI,
    const label startCut,
    const label faceI,
...
) const
{
    ...
    {
        WarningIn("Foam::multiCellCuts::walkFace")
...
bool Foam::multiCellCuts::walkCell
(
    const label cellI,
    const label startCut,
    const label faceI,
...
// one found.
void Foam::multiCellCuts::calcCellLoops(const labelList& cutCells)
{
...
void Foam::multiCellCuts::walkEdges
(
    const label cellI,
    const label pointI,
    const label status,
...
Foam::labelList Foam::multiCellCuts::nonAnchorPoints
(
...
bool Foam::multiCellCuts::loopAnchorConsistent
(
    const label cellI,
...
bool Foam::multiCellCuts::calcAnchors
(
    const label cellI,
    const labelList& loop,
    const pointField& loopPts,
...
) const
{
    ...
    if (uncutIndex == -1)
    {
        WarningIn("Foam::multiCellCuts::calcAnchors")
    ...
    // set1.
    {
        forAll(cFaces, i)
        {
            ...
            forAll(f, fp)
            {
                ...
                {
                    FatalErrorIn("Foam::multiCellCuts::calcAnchors")
                        << abort(FatalError);
                }
...
Foam::pointField Foam::multiCellCuts::loopPoints
(
    const labelList& loop,
...
Foam::scalarField Foam::multiCellCuts::loopWeights(const labelList& loop) const
{
...
bool Foam::multiCellCuts::validEdgeLoop
(
    const labelList& loop,
    const scalarField& loopWeights
) const
...
Foam::label Foam::multiCellCuts::countFaceCuts
(
    const label faceI,
    const labelList& loop
) const
...
bool Foam::multiCellCuts::conservativeValidLoop
(
    const label cellI,
    const labelList& loop
) const
...
bool Foam::multiCellCuts::validLoop
(
    const label cellI,
    const labelList& loop,
    const scalarField& loopWeights,
...
) const
{
    ...
    {
        WarningIn("Foam::multiCellCuts::validLoop")
...
void Foam::multiCellCuts::setFromCellLoops()
{
    ...
    forAll(cellLoops_, cellI)
    {
        ...
        if (loop.size())
        {
            ...
            )
            {
                ...
                //FatalErrorIn("multiCellCuts::setFromCellLoops()")
                WarningIn("multiCellCuts::setFromCellLoops")
                    << "Illegal loop " << loop
...
// was valid.
bool Foam::multiCellCuts::setFromCellLoop
(
    const label cellI,
    const labelList& loop,
    const scalarField& loopWeights
...
void Foam::multiCellCuts::setFromCellCutter
(
...
void Foam::multiCellCuts::orientPlanesAndLoops()
{
...
void Foam::multiCellCuts::calcLoopsAndAddressing(const labelList& cutCells)
{
    ...
    forAll(edgeIsCut_, edgeI)
    {
        if (edgeIsCut_[edgeI])
        {
            ...
            if (weight < 0 || weight > 1)
            {
                FatalErrorIn
                (
                    "multiCellCuts::calcLoopsAndAddressing(const labelList&)"
...
void Foam::multiCellCuts::check() const
{
    ...
    forAll(edgeIsCut_, edgeI)
    {
        if (edgeIsCut_[edgeI])
        {
            ...
            )
            {
                ...
                //FatalErrorIn("multiCellCuts::check()")
                WarningIn("multiCellCuts::check()")
                    << "edge:" << edgeI << " vertices:"
                    << mesh().edges()[edgeI]
...
Foam::multiCellCuts::multiCellCuts
(
    const polyMesh& mesh,
    const triSurface& surf,
    const labelList& meshVerts,
    const labelList& vertTris,
    const labelList& meshEdges,
    const labelList& edgeTris,
    ...
    cellAnchorPoints_(mesh.nCells())
{
    if (debug)
    {
        Pout<< "multiCellCuts : constructor from cellLoops" << endl;
    }
    ...
    if (debug)
    {
        Pout<< "multiCellCuts : leaving constructor from cellLoops" << endl;
    }
}
...
void Foam::multiCellCuts::clearOut()
{
...
Foam::pointField Foam::multiCellCuts::loopPoints(const label cellI) const
{
    const labelList& loop = cellLoops_[cellI];
...
// Flip loop for cell
void Foam::multiCellCuts::flip(const label cellI)
{
    labelList& loop = cellLoops_[cellI];
...
// Flip loop only
void Foam::multiCellCuts::flipLoopOnly(const label cellI)
{
    labelList& loop = cellLoops_[cellI];
